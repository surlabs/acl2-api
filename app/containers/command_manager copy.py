...
class CommandManager:
    ...
    async def update_command_info_last_interaction(self, object_id: str) -> None|CommandInfo:
        res = None
        container_info = await command_repo.find_one(object_id)
        if container_info is not None:
            container_info = container_info.update()
            res = await command_repo.save(container_info)
            return res
        else:
    ...
    async def read_lines_acl2(self, user_id: str):
        output = []
        while True:
            logger.info(f"self.containers[user_id].proccess: {self.containers[user_id].proccess}--- {self.containers[user_id].proccess == None}")
            ...
            if user_id is not None and self.containers.get(user_id) is not None:
                ...
                with self.containers[user_id].lock:
                    if self.containers[user_id].proccess.poll() is not None:
                        ...
                        output = await self.read_lines_acl2(user_id)
                        await self.update_command_info_last_interaction(self.containers[user_id].object_id)
